...
describe("dashboard guard API authentication", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    process.env.NINEROUTER_PEER_TOKEN = PEER_TOKEN;
    mocks.getSettings.mockResolvedValue({ requireLogin: true });
  ...
  it("rejects a dashboard API request without a JWT", async () => {
  ...
  it("allows a dashboard API request with a valid JWT", async () => {
    mocks.verifyDashboardAuthToken.mockResolvedValue(true);

    const apiRequest = request("/api/keys", { host: "router.example.com" }, "valid-jwt");
