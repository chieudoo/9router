...
XProxy is a local AI routing gateway and dashboard built on Next.js.
...
flowchart LR
    ...
    subgraph Router[XProxy Local Process]
...
- `src/lib/localDb.js`
- file: `${DATA_DIR}/db.json` (or `~/.xproxy/db.json` when `DATA_DIR` is unset)
- entities: providerConnections, providerNodes, modelAliases, combos, apiKeys, settings, pricing

Usage DB:

- `src/lib/usageDb.js`
- files: `~/.xproxy/usage.json`, `~/.xproxy/log.txt`
...
- main state: `${DATA_DIR}/db.json` (or `~/.xproxy/db.json`)
- usage stats: `~/.xproxy/usage.json`
- request log lines: `~/.xproxy/log.txt`
...
flowchart LR
    ...
    subgraph ContainerOrProcess[XProxy Runtime]
        Next[Next.js Server\nPORT=20128]
...
1. `usageDb` currently stores under `~/.xproxy` and does not follow `DATA_DIR`.
...
- Build from source: `cd /root/dev/xproxy && npm run build`
- Build Docker image: `cd /root/dev/xproxy && docker build -t xproxy .`
