...
function ensureClientToolsScript() {
  const scriptPath = path.join(os.tmpdir(), "xproxy-devin-client-tools.mjs");
  ...
  async execute({ model, body, credentials, signal, log }) {
    ...
    const sseStream = new ReadableStream({
      start(controller) {
        ...
        // modify the filesystem on the host running xproxy — only expose locally.
        ...
        sendRpc("initialize", {
          protocolVersion: "0.3",
          clientInfo: { name: "xproxy", version: "1.0" },
          capabilities: {},
        });
      },
    });
