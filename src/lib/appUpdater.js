  ...
  try {
    const mitmPidFile = path.join(
      process.platform === "win32"
        ? path.join(process.env.APPDATA || "", "xproxy")
        : path.join(os.homedir(), ".xproxy"),
      "mitm",
      ".mitm.pid"
    );
...
// Collect PIDs of all xproxy-related processes (excluding current)
  ...
  if (platform === "win32") {
    try {
      ...
      lines.forEach(line => {
        ...
        // Match anything running from xproxy install dir or wrapper cli.js
        const isAppProcess = lower.includes("xproxy") ||
    ...
    try {
      ...
      output.split("\n").forEach(line => {
        const isAppProcess = line.includes("xproxy") ||
...
function getDataDir() {
  ...
  if (process.platform === "win32") {
    return path.join(process.env.APPDATA || path.join(os.homedir(), "AppData", "Roaming"), "xproxy");
  }
  return path.join(os.homedir(), ".xproxy");
}
...
// Resolve npx/xproxy binary to relaunch after update (cross-platform)
function resolveRelaunchCommand() {
  const isWin = process.platform === "win32";
  // Prefer `npx xproxy` — works regardless of global bin path changes after npm i -g
