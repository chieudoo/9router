...
const previousDefaultRegExp = /^# xproxy-prev-default = "([^"]*)"[ \t]*\r?\n?/m;
const previousSubagentRegExp = (type) =>
  new RegExp(
    `^# xproxy-prev-subagent-${escapeRegExp(type)} = "([^"]*)"[ \\t]*\\r?\\n?`,
    "m",
  );
...
function buildModelSection({ slot, model, baseUrl, apiKey, contextWindow, name }) {
  const lines = [
    ...
    `description = ${tomlString("Routed via XProxy gateway")}`,
...
function rememberPreviousDefault(toml) {
  ...
  return insertMarker(toml, `# xproxy-prev-default = ${tomlString(current)}\n`);
}
...
function rememberPreviousSubagent(toml, type) {
  ...
  return insertMarker(
    toml,
    `# xproxy-prev-subagent-${type} = ${tomlString(previous)}\n`,
  );
}
