...
// (prevents EBUSY on `npm i -g xproxy@latest` while MITM is running).
...
const ENCRYPT_SALT = "xproxy-mitm-pwd";
...
async function startServer(apiKey, sudoPassword, forceKillPort443 = false) {
  ...
  if (!effectiveServerPath || !fs.existsSync(/* turbopackIgnore: true */ effectiveServerPath)) {
    ...
    if (!effectiveServerPath || !fs.existsSync(/* turbopackIgnore: true */ effectiveServerPath)) {
      throw new Error(`MITM server.js not found at ${effectiveServerPath}. Reinstall xproxy.`);
    }
  }
