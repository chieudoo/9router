...
// Check if settings has XProxy customModels
const hasXProxyConfig = (settings) => {
  if (!settings || !settings.customModels) return false;
  return settings.customModels.some(m => m.id?.startsWith("custom:XProxy"));
};
...
// POST - Update XProxy customModels (merge with existing settings)
...
export async function POST(request) {
  try {
    ...
    // Remove all existing XProxy configs
    settings.customModels = settings.customModels.filter(m => !m.id?.startsWith("custom:XProxy"));
...
// DELETE - Remove XProxy customModels only (keep other settings)
