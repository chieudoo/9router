...
const hasXProxyConfig = (config) => {
  ...
  return config.some((entry) => entry.name === "XProxy");
};

const getXProxyEntry = (config) => {
  if (!Array.isArray(config)) return null;
  return config.find((entry) => entry.name === "XProxy") || null;
};
...
export async function GET() {
  try {
    ...
    const entry = getXProxyEntry(config);
...
// POST - Apply XProxy config to chatLanguageModels.json
export async function POST(request) {
  try {
    ...
    // Replace existing XProxy entry or append
    const idx = config.findIndex((e) => e.name === "XProxy");
    if (idx >= 0) {
      config[idx] = newEntry;
    } else {
...
// DELETE - Remove XProxy entry from chatLanguageModels.json
export async function DELETE() {
  try {
    ...
    config = config.filter((e) => e.name !== "XProxy");
    await fs.writeFile(configPath, JSON.stringify(config, null, 2));

    return NextResponse.json({
      success: true,
      message: "XProxy removed from Copilot config",
    });
  } catch (error) {
