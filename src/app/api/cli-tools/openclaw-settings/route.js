...
// Check if settings has XProxy config
const hasXProxyConfig = (settings) => {
  if (!settings || !settings.models || !settings.models.providers) return false;
  return !!settings.models.providers["xproxy"];
};

// Read per-agent models.json and return current model id (without "xproxy/" prefix)
const readAgentModel = async (agentDir) => {
  try {
    const modelsPath = path.join(agentDir, "models.json");
    const content = await fs.readFile(modelsPath, "utf-8");
    const data = JSON.parse(content);
    const models = data?.providers?.["xproxy"]?.models;
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      hasXProxy: hasXProxyConfig(settings),
...
// POST - Update XProxy settings (merge with existing settings)
export async function POST(request) {
  try {
    ...
    const fullModelId = `xproxy/${model}`;

    // Remove all old xproxy/* entries from agents.defaults.models
    Object.keys(settings.agents.defaults.models)
      .filter((k) => k.startsWith("xproxy/"))
    ...
    // Add fresh xproxy models to allowlist
    allModelIds.forEach((m) => {
      settings.agents.defaults.models[`xproxy/${m}`] = {};
    });

    // Remove old xproxy model from each agent in agents.list. The
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        if (resolveAgentModel(agent.model).startsWith("xproxy/")) {
    ...
    // Update models.providers.xproxy with all models
    settings.models.providers["xproxy"] = {
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        ...
        if (agentModel) return { ...agent, model: `xproxy/${agentModel}` };
        return agent;
      });
...
// DELETE - Remove XProxy settings only (keep other settings)
export async function DELETE() {
  try {
    ...
    // Remove XProxy from models.providers
    if (settings.models && settings.models.providers) {
      delete settings.models.providers["xproxy"];
    ...
    // Remove xproxy models from agents.defaults.models allowlist
    if (settings.agents?.defaults?.models) {
      const keysToRemove = Object.keys(settings.agents.defaults.models).filter((k) => k.startsWith("xproxy/"));
    ...
    // Reset agents.defaults.model.primary if it uses xproxy
    if (settings.agents?.defaults?.model?.primary?.startsWith("xproxy/")) {
    ...
    return NextResponse.json({
      success: true,
      message: "XProxy settings removed successfully",
    });
  } catch (error) {
