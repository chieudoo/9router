...
const getProviderEnvPath = () => {
  ...
  return path.join(configDir, "jcode", "provider-xproxy.env");
};
...
const hasXProxyConfig = (config) => {
  ...
  if (providers["xproxy"]) return true;

  for (const [name, provider] of Object.entries(providers)) {
    if (provider.base_url && provider.base_url.includes(`localhost:${process.env.PORT}`)) {
      return true;
    }
  }
...
export async function GET() {
  ...
  const hasXProxy = hasXProxyConfig(config);
...
export async function POST(request) {
  try {
    ...
    env.JCODE_XPROXY_API_KEY = apiKey;
    await writeProviderEnv(env);

    return NextResponse.json({
      success: true,
      message: "jcode configured successfully. Use: jcode --provider-profile xproxy",
      configPath: getConfigPath(),
    });
  } catch (error) {
...
export async function DELETE() {
  try {
    ...
    delete config.providers["xproxy"];

    await writeConfig(config);

    const env = await readProviderEnv();
    delete env.JCODE_XPROXY_API_KEY;
    await writeProviderEnv(env);

    return NextResponse.json({
      success: true,
      message: "xproxy configuration removed from jcode",
    });
  } catch (error) {
