...
const hasXProxyConfig = (auth) => {
  if (!auth) return false;
  const entry = auth["openai-compatible"] || auth["xproxy"];
  if (!entry) return false;
  const baseUrl = entry.baseUrl || entry.baseURL || "";
  return baseUrl.includes("localhost") || baseUrl.includes("127.0.0.1") || baseUrl.includes("xproxy");
};
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      hasXProxy: hasXProxyConfig(auth),
...
export async function POST(request) {
  try {
    ...
    try {
      ...
      vscode["kilocode.customProvider"] = { name: "XProxy", baseURL: normalizedBaseUrl, apiKey };
...
export async function DELETE() {
  try {
    ...
    return NextResponse.json({ success: true, message: "XProxy settings removed from Kilo Code" });
  } catch (error) {
