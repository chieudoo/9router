          ...
          {!checking && deepseekStatus?.installed && (
            <>
                  ...
                  <div className="flex flex-col gap-2 mb-2">
                    {tool.notes.map((note, idx) => (
                      <div key={idx} className={`flex items-start gap-2 p-2 rounded text-xs ${
                        ...
                        <Icon name={note.type === "warning" ? "warning" : note.type === "error" ? "error" : "info"} className="text-[14px] mt-0.5" />
                        <span>{note.text}</span>
                      </div>
                    ))}
                  </div>
              ...
              <div className="grid grid-cols-1 gap-2 sm:flex sm:items-center">
                ...
                <Button variant="outline" size="sm" onClick={handleReset} disabled={!deepseekStatus?.hasXProxy} loading={restoring}>
