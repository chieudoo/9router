...
export default function OpenCodeToolCard({ tool, isExpanded, onToggle, baseUrl, apiKeys, activeProviders, initialStatus, tunnelEnabled, tunnelPublicUrl, tailscaleEnabled, tailscaleUrl }) {
  ...
  const getConfigStatus = () => {
    ...
    if (!status.hasXProxy) return "not_configured";
    const url = status.config?.provider?.["xproxy"]?.options?.baseURL || "";
  ...
  const getManualConfigs = () => {
    ...
    return [{
      ...
      content: JSON.stringify({
        ...
        model: `xproxy/${activeModelToShow}`,
      }, null, 2),
    }];
  };
              ...
              <div className="flex flex-col gap-3 p-4 bg-yellow-500/10 border border-yellow-500/30 rounded-lg">
                ...
                <div className="flex items-center gap-2 pl-9">
                  ...
                  <Button variant="outline" size="sm" onClick={() => setShowInstallGuide(!showInstallGuide)}>
                    <Icon name={showInstallGuide ? "expand_less" : "help"} className="text-[18px] mr-1" />
          ...
          {!checking && status?.installed && (
            <>
              <div className="flex flex-col gap-2">
                ...
                {/* Current configured */}
                {status?.config?.provider?.["xproxy"]?.options?.baseURL && (
                ...
                <div className="grid grid-cols-1 gap-1.5 sm:grid-cols-[8rem_auto_1fr] sm:items-start sm:gap-2">
                  ...
                  <Icon name="arrow_forward" className="text-text-muted text-[14px] mt-1.5" />
                    ...
                    <div className="flex flex-wrap gap-1.5 min-h-[28px] px-2 py-1.5 bg-surface rounded border border-border">
                      {selectedModels.length === 0 ? (
                        ...
                        selectedModels.map((model) => (
                          ...
                          >
                            {model === activeModel && <Icon name="star" className="text-[10px]" />}
                            {model}
                            <button
                              onClick={async (e) => {
                                e.stopPropagation();
              ...
              <div className="grid grid-cols-1 gap-2 sm:flex sm:items-center">
                ...
                <Button variant="outline" size="sm" onClick={handleReset} disabled={!status.hasXProxy} loading={restoring}>
